...
| 1.13 | — | **Reserved** for the concurrent `feat/14-analytics-infra` branch, which numbers its §14 amendment 1.13. Recorded here so the 1.12 -> 1.14 gap reads as deliberate rather than as a lost row, and so it stays legible if that branch is abandoned. Nothing in §8.2 depends on it |
