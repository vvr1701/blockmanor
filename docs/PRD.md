...
**Version:** 1.14 · **Date:** 2026-08-12 · **Status:** Approved for build
...
- **`engineVersion`.** The document records a fingerprint of the **re-simulation surface**, captured at generation, e.g. `"daily-sim-v1+990bd596"`. Same rationale as the frozen `engineConfig`: everything §8.5's re-simulation depends on must travel with the board, and engine *code* is such a dependency. Deploys that can change engine behaviour are restricted to the post-generation window by `docs/RELEASING.md`.
  - **What it covers.** The surface is exactly what §8.5 needs to reproduce a submitted daily run: `packages/engine` behaviour on the daily path (fixed `pieceSequence` draws, placement, clearing, scoring, combo, ivy, board death, sequence exhaustion) **plus** the `dailyGameConfig()` builder and every literal it hardcodes (`mode`, `seedSalt`, `goals`, `mercy`, `stars`, `ivySpreadInterval`, `ivyMaxTiles`).
  - **How it is derived.** Computed, never hand-written: fixed probe boards — literal tuning, literal prefill, literal sequences drawn from the §6.2 piece table, one ending in exhaustion and one in board death — are played out move by move under a fixed policy, and the whole trace (rebuilt `GameConfig`, every move, every emitted `GameEvent`, and the `simulate()` result over the collected log) is hashed. The prefix is a scheme tag; bumping it is how a change to the probe itself is told apart from a change to the engine.
  ...
  - **Not the §5 determinism corpus hash.** That hash pins engine behaviour on *generated* piece streams and its fuzz corpus sets no `pieceSequence` at all, so it cannot move for a change to the fixed-sequence path the Daily Board plays entirely from. The two are complementary and both are pinned in CI.
