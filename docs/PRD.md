...
**Version:** 1.12 · **Date:** 2026-08-10 · **Status:** Approved for build
...
- **`engineVersion`.** The document records the `packages/engine` package version plus the pinned determinism corpus hash (§5 Stage-0 DoD), captured at generation, e.g. `"0.1.0+392ad7a4"`. Same rationale as the frozen `engineConfig`: everything §8.5's re-simulation depends on must travel with the board, and engine *code* is such a dependency. Deploys that can change engine behaviour are restricted to the post-generation window by `docs/RELEASING.md`.
...
*End of PRD v1.11 — amendments require a changelog entry (§0).*
